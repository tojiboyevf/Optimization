...
def get_parser():
    ...
    parser.add_argument('--optim', default='sgd', type=str, help='optimizer',
                        choices=['sgd', 'adam', 'adabelief', 'acprop',
                                 ])
    ...
    parser.add_argument('--scheduler', dest='scheduler', action='store_true', help='using scheduler')
...
def get_ckpt_name(model='resnet18', optimizer='sgd', lr=0.1, momentum=0.9,
                  beta1=0.9, beta2=0.999, eps=1e-8, weight_decay=5e-4,
                  reset = False, run = 0, scheduler = False):
    name = {
        'sgd': 'lr{}-momentum{}-wdecay{}-scheduler{}-run{}'.format(lr, momentum, weight_decay, scheduler, run),
        ...
        'acprop': 'lr{}-betas{}-{}-eps{}-wdecay{}-run{}'.format(lr, beta1, beta2, eps, weight_decay, run),
    }[optimizer]
...
def create_optimizer(args, model_params):
    ...
    if args.optim == 'sgd':
        ...
        return AdaBelief(model_params, args.lr, betas=(args.beta1, args.beta2),
                          weight_decay=args.weight_decay, eps=args.eps)
    elif args.optim == 'acprop':
        return ACProp(model_params, args.lr, betas=(args.beta1, args.beta2),
                          weight_decay=args.weight_decay, eps=args.eps, rectify = False, weight_decouple = False, amsgrad = False, 
...
def main():
    ...
    if args.resume:
        ...
        curve = os.path.join('curves', ckpt_name)     
    ...
    for epoch in range(start_epoch + 1, args.total_epoch):
        ...
        if not os.path.isdir('curves'):
            os.mkdir('curves')
        torch.save({'train_acc': train_accuracies, 'test_acc': test_accuracies, 'train_loss' : train_losses, 'test_loss' : test_losses},
                   os.path.join('curves', ckpt_name))
